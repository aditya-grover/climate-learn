...
from .components.pos_embed import get_1d_sincos_pos_embed_from_grid, get_2d_sincos_pos_embed, interpolate_pos_embed
...
class ClimaX(nn.Module):
    ...
    def load_pretrained_weights(self, pretrained_path):
        ...
        checkpoint_model = {k[4:]: v for k, v in checkpoint_model.items()}
        # interpolate positional embedding
        interpolate_pos_embed(self, checkpoint_model, new_size=self.img_size)
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        ...
        lead_times = torch.ones(x.shape[0]).to(torch.long) * lead_times.hours() / 100
        lead_times = lead_times.to(device=x.device, dtype=x.dtype)
