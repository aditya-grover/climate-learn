...
def test_crps_gaussian():
    ...
    w_lat = (torch.from_numpy(w_lat).unsqueeze(0).unsqueeze(-1).to(dtype=crps_score.dtype, device=crps_score.device))

    loss_dict_ps = {}
    with torch.no_grad():
        for i, var in enumerate(vars):
            loss_dict_ps[f"w_crps_{var}_{log_postfix}"] = torch.mean(crps_score[:, i] * w_lat)

    loss_dict_ps["w_crps"] = np.mean([loss_dict_ps[k].cpu() for k in loss_dict_ps.keys()])
