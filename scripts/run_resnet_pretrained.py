...
def main():
    ...
    parser.add_argument('--config', default='resnet_pretrained')
    args = parser.parse_args()
    cfg = load_config(f'configs/{args.config}.yaml')
    
    dm = load_data_numpy(cfg)

    model = cl.models.hub.ResNetPretrained(
    ...
    wandb.init(
        project='Climate',
        name=f'Resnet Pretrained, Pretrained Backbone = {cfg["use_pretrained_weights"]}, Layer1,2,3(1)',
        config=cfg,
    )
    logger = WandbLogger()
