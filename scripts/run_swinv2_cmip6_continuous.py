...
def main():
    with open('scripts/configs/config_cmip6_swinv2_stage1.yaml') as f:
        cfg = yaml.safe_load(f)

    pretrained_weights = cfg['pretrained_weights'].split('/')[1]
    
    default_root_dir=f"{cfg['default_root_dir']}/swinv2_classification_{pretrained_weights}_{cfg['embed_type']}_emb_pretrained_{cfg['use_pretrained_weights']}/"
    ...
    if cfg['ckpt_dir'] is not None:
        ckpt_path = get_best_checkpoint(f"{cfg['ckpt_dir']}/swinv2_classification_{pretrained_weights}_{cfg['embed_type']}_emb_pretrained_{cfg['use_pretrained_weights']}/")
    ...
    wandb.init(
        ...
        name=f"{cfg['model'].upper()}, Pretrained Backbone = {cfg['use_pretrained_weights']} Stage = {cfg['stage']}, Model = {cfg['pretrained_weights']}, Sinusoidal Emb", 
        config=cfg
    )
