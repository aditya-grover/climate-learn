...
def get_resnet_cont_value(data, variable, scale):
    x = data[scale][variable]["ResNet-continuous"]["x"]
    y = data[scale][variable]["ResNet-continuous"]["y"]
    return x, y

def get_resnet_iter_value(data, variable, scale):
    x = data[scale][variable]["ResNet-iterative"]["x"]
    y = data[scale][variable]["ResNet-iterative"]["y"]
    return x, y
...
def get_unet_cont_value(data, variable, scale):
    x = data[scale][variable]["UNet-continuous"]["x"]
    y = data[scale][variable]["UNet-continuous"]["y"]
    return x, y

def get_unet_iter_value(data, variable, scale):
    x = data[scale][variable]["UNet-iterative"]["x"]
    y = data[scale][variable]["UNet-iterative"]["y"]
    return x, y
...
def get_vit_cont_value(data, variable, scale):
    x = data[scale][variable]["ViT-continuous"]["x"]
    y = data[scale][variable]["ViT-continuous"]["y"]
    return x, y

def get_vit_iter_value(data, variable, scale):
    x = data[scale][variable]["ViT-iterative"]["x"]
    y = data[scale][variable]["ViT-iterative"]["y"]
    return x, y
...
def plot_rmse(axs, data, variable, resolution):
    ...
    axs.plot(*get_resnet_cont_value(data, variable, resolution), marker="o", markersize=2.5, color="C2", label=f"ResNet-cont")
    # axs.plot(*get_unet_cont_value(data, variable, resolution), marker="x", markersize=2.5, color="C2", alpha=0.7, linestyle='dashed', label=f"UNet-cont")
    # axs.plot(*get_vit_cont_value(data, variable, resolution), marker="x", markersize=2.5, color="C3", alpha=0.7, linestyle='dashed', label=f"ViT-cont")

    axs.plot(*get_resnet_iter_value(data, variable, resolution), marker="o", markersize=2.5, color="C3", label=f"ResNet-iter")
    # axs.plot(*get_unet_iter_value(data, variable, resolution), marker="s", markersize=2.5, color="C2", alpha=0.6, linestyle='dotted', label=f"UNet-iter")
    # axs.plot(*get_vit_iter_value(data, variable, resolution), marker="s", markersize=2.5, color="C3", alpha=0.6, linestyle='dotted', label=f"ViT-iter")
...
@click.option('--output', '-o', default="table2.pdf")
def main(var, rmsefile, accfile, metric, resolution, output):
    ...
    fig.legend(*axs[0, 0].get_legend_handles_labels(), loc='lower center', ncol=3, borderaxespad=0.,   bbox_to_anchor = (0.5, -0.05),
