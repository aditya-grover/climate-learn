...
def main():
    ...
    subsample = Hours(1)
    batch_size = 128
    default_root_dir=f"results_rebuttal/resnet_prob_ensemble_{args.pred_range}"
    ...
    pretrained_prefix = f'/home/tungnd/climate-learn/results_rebuttal/resnet_{args.pred_range}_ensemble'
    
    model = cl.models.hub.ResNetEnsemble(resnet_base_args, pretrained_prefix)
    ...
    resnet_ensemble = cl.load_prob_forecasting_module(
        data_module=dm,
        model=model,
        optim=optimizer,
        sched=lr_scheduler
    ...
    trainer = cl.Trainer(
        early_stopping="val/lat_crps_gaussian:aggregate",
        patience=5,
        accelerator="gpu",
        devices=[args.gpu],
        precision=16,
    ...
    trainer.test(resnet_ensemble, datamodule=dm)
